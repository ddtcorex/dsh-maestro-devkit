...
- `src/host/capture.ts` — `capture` — 3 viewports + DOM + geometry, tunnel-aware (`local` / `https://dsh-company.ddtcorex.com`).
...
- **Tunnel-aware**: `targetUrl: auto` resolves to local `:3080` or `https://dsh-company.ddtcorex.com` via config; capture/HMR/inspect verify both.
...
- Local design: `docs/specs/2026-08-28-dsh-devkit-design.md` (8 sections, tunnel-aware, 5 Hard Rules)
