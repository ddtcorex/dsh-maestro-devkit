export async function cordisInspect(opts, ctx) {
    ...
    if (opts.service)
        return { service: opts.service, contract: `contract for ${opts.service}` };
    if (opts.event)
        return { event: opts.event, contract: `contract for ${opts.event}` };
    if (opts.slot)
        return { slot: opts.slot, occupants: [] };
    return {};
}
export async function sessionInspect(opts, ctx) {
    ...
    return { cwd: ctx.exec?.agent?.session?.header?.cwd ?? process.cwd(), sessionId: opts.sessionId };
}
