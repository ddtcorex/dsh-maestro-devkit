...
function OverlayToolbar({ onCapture, onInspect }) {
    return React.createElement('div', { style: { position: 'fixed', top: 8, right: 8, zIndex: 9999, background: 'white', border: '1px solid #ddd', borderRadius: 8, padding: 8, display: 'flex', gap: 8 } }, React.createElement('button', { onClick: onCapture }, 'Capture'), React.createElement('button', { onClick: onInspect }, 'Inspect'), React.createElement('span', null, 'DevKit Overlay'));
}
function Overlay() {
...
function apply(ctx) {
    ctx.effect(() => {
        ...
        if (!slots?.inject)
            return () => { };
        const dispose = slots.inject('shell:overlay', () => React.createElement(overlay_js_1.OverlayToolbar, {}));
        return () => {
            try {
                dispose?.();
            }
