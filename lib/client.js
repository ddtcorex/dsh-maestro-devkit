...
function OverlayToolbar({ onCapture, onInspect }) {
    ...
    return React.createElement('div', {
        ...
        onMouseEnter: (e) => (e.currentTarget.style.background = 'var(--dsw-alias-interactive-bg-hover)'),
        onMouseLeave: (e) => (e.currentTarget.style.background = 'var(--dsw-alias-bg-base)'),
    }, 'Inspect'), React.createElement('span', { style: { color: 'var(--dsw-alias-label-secondary)', padding: '0 3px' } }, 'DevKit'));
}
function Overlay() {
...
function apply(ctx) {
    ctx.effect(() => {
        ...
        const callHost = async (action) => {
            try {
                let res;
                if (action === 'capture' && host?.call)
                    res = await host.call('devkit.capture', {});
                else if (action === 'inspect' && host?.call)
                    res = await host.call('devkit.inspect', {});
                else {
                    const conn = ctx.get?.('connection');
                    if (conn?.call)
                        res = await conn.call('/dsh-maestro-devkit', { action });
                }
