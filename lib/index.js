...
export const inject = ['tools', 'harness'];
export function apply(ctx, config = {}) {
    ...
    ctx.effect(() => {
        const harness = ctx.get?.('harness') ?? ctx.harness;
        if (!harness?.handle)
            return () => { };
        const disposers = [];
        disposers.push(harness.handle('devkit.capture', async (payload) => {
            return await capture({ targetUrl: payload?.targetUrl ?? config.targetUrl ?? 'auto' });
        }));
        disposers.push(harness.handle('devkit.inspect', async (payload) => {
