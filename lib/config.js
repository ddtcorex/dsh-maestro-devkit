 ...
 */
export async function resolveTargetUrl(input) {
    if (process.env.DSH_FRONTEND_URL)
        return process.env.DSH_FRONTEND_URL;
    if (input && input !== 'auto' && input !== 'local' && input !== 'tunnel')
        return input;
    if (input === 'local')
    ...
    if (input === 'tunnel' || input === 'auto' || !input) {
        try {
            ...
            if (fs.existsSync(p)) {
                ...
                const t = j?.['dsh-maestro-remote']?.tunnelUrl || j?.remote?.tunnelUrl || j?.['dsh-maestro-devkit']?.targetUrl;
