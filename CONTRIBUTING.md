# Contributing to dsh-maestro-devkit

Thank you for contributing to **dsh-maestro-devkit** (`@ddtcorex/dsh-maestro-devkit`) — General development toolkit for DeepSeek Harness — visual review, HMR, style inspector, Cordis/Govard/Skills dev (tunnel-aware), packaged as a Cordis plugin.

## Getting Started

1. **Fork and clone** `github.com/ddtcorex/dsh-maestro-devkit`.
...
- **Scope:** optional, without the `dsh-maestro-` prefix — e.g. `feat(memory):`, `fix(snapshot):`, `docs(readme):`
...
pnpm test        # vitest run (14 files, 211 tests)
pnpm build       # tsc host + client && node scripts/build-client.mjs -> lib/
test -f lib/index.js && echo "build ok"
```

After touching the client bundle, verify on live DSH Web (`:3080`), not just curl/grep. Snapshot regression — "keep project/daily logs out of snapshot" — must stay green.
...
- Security vulnerabilities: use GitHub's private advisory reporting at `https://github.com/ddtcorex/dsh-maestro-devkit/security/advisories` — do not file a public issue.
